...
export function TaxChatInput({ value, onChange, onSend, onUploadClick, disabled }: Props) {
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-end gap-3 bg-white/85 border border-white/70 rounded-[1.5rem] px-5 py-2 shadow-sm">
          <button
            ...
            disabled={disabled}
            title="Upload document (CAS, P&L, Holdings, ITR)"
            className="w-9 h-9 rounded-full bg-minto-accent/10 text-minto-accent flex items-center justify-center shrink-0 disabled:opacity-30 hover:bg-minto-accent/20 transition-colors mb-0.5"
          >
            <Paperclip size={17} />
          </button>
          <textarea
            ...
            placeholder="Ask me anything or type a number to answer…"
            disabled={disabled}
            rows={1}
        ...
        <p className="text-center text-[10px] text-minto-text-muted mt-2">
          Minto provides informational insights, not tax advice. Consult a CA for complex situations.
        </p>
      </div>
    </div>
  );
