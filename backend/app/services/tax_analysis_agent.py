...
Entry point: stream_tax_analysis() — synchronous iterator, called by the router
via _stream_sse(). Yields tokens from Gemini as they arrive (RunEvent.run_content).

For the initial analysis: builds context from intake_answers + tax_docs, runs the
4-step analyst prompt.
For follow-up questions: passes conversation history as proper role/content message
pairs so the agent gives a targeted answer instead of re-running the full analysis.
"""
